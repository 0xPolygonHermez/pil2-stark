...
public:
    ...
    inline void loadPolynomials(StepsParams& params, ParserArgs &parserArgs, std::vector<Dest> &dests, __m256i *bufferT_, uint64_t row, bool domainExtended) {
        uint64_t nOpenings = setupCtx.starkInfo.openingPoints.size();
    ...
    inline void copyPolynomial(__m256i* destVals, bool inverse, uint64_t destId, uint64_t destDim, __m256i* tmp1, Goldilocks3::Element_avx* tmp3) {
        if(destDim == 1) {
            if(inverse) {
                ...
                Goldilocks::copy_avx(destVals[0],tmp1[destId]);
            }
        } else if(destDim == 3) {
            if(inverse) {
                ...
                Goldilocks::copy_avx(destVals[0], tmp3[destId][0]);
                Goldilocks::copy_avx(destVals[1],tmp3[destId][1]);
                Goldilocks::copy_avx(destVals[2],tmp3[destId][2]);
            }
        }
    }
    ...
    inline void storePolynomial(std::vector<Dest> dests, __m256i* destVals, uint64_t row) {
        for(uint64_t i = 0; i < dests.size(); ++i) {
            if(dests[i].params[0].parserParams.destDim == 1) {
                ...
                Goldilocks::store_avx(&dests[i].dest[row*offset], uint64_t(offset), destVals[i*FIELD_EXTENSION]);
            } else {
                uint64_t offset = dests[i].params[0].offset != 0 ? dests[i].params[0].offset : FIELD_EXTENSION;
                Goldilocks::store_avx(&dests[i].dest[row*offset], uint64_t(offset), destVals[i*FIELD_EXTENSION]);
                Goldilocks::store_avx(&dests[i].dest[row*offset + 1], uint64_t(offset), destVals[i*FIELD_EXTENSION + 1]);
                Goldilocks::store_avx(&dests[i].dest[row*offset + 2], uint64_t(offset), destVals[i*FIELD_EXTENSION + 2]);
            }
        }
    }
    ...
    void calculateExpressions(StepsParams& params, ParserArgs &parserArgs, std::vector<Dest> dests, bool domainExtended) override {
        ...
        __m256i** numbers_ = new __m256i*[dests.size()];
        for(uint64_t i = 0; i < dests.size(); ++i) {
            ...
            for(uint64_t j = 0; j < dests[i].params[0].parserParams.nNumbers; ++j) {
                numbers_[i][j] = _mm256_set1_epi64x(numbers[j]);
            }
        }
    ...
    #pragma omp parallel for
        for (uint64_t i = 0; i < domainSize; i+= nrowsPack) {
            ...
            __m256i destVals[dests.size()*FIELD_EXTENSION];
            ...
            for(uint64_t j = 0; j < dests.size(); ++j) {
                ...
                __m256i tmp1[dests[j].params[0].parserParams.nTemp1];
                Goldilocks3::Element_avx tmp3[dests[j].params[0].parserParams.nTemp3];
                ...
                for (uint64_t kk = 0; kk < dests[j].params[0].parserParams.nOps; ++kk) {
                    switch (ops[kk]) {
                        ...
                        case 4: {
                            ...
                            Goldilocks::op_avx(args[i_args], tmp1[args[i_args + 1]], bufferT_[nColsStagesAcc[args[i_args + 2]] + args[i_args + 3]], numbers_[j][args[i_args + 4]]);
                            i_args += 5;
                            break;
                        }
                        case 5: {
                        ...
                        case 8: {
                            ...
                            Goldilocks::op_avx(args[i_args], tmp1[args[i_args + 1]], tmp1[args[i_args + 2]], numbers_[j][args[i_args + 3]]);
                            i_args += 4;
                            break;
                        }
                        case 9: {
                        ...
                        case 11: {
                            ...
                            Goldilocks::op_avx(args[i_args], tmp1[args[i_args + 1]], publics[args[i_args + 2]], numbers_[j][args[i_args + 3]]);
                            i_args += 4;
                            break;
                        }
                        case 12: {
                           // COPY number to tmp1
                            Goldilocks::copy_avx(tmp1[args[i_args]], numbers_[j][args[i_args + 1]]);
                            i_args += 2;
                            break;
                        }
                        case 13: {
                            // OPERATION WITH DEST: tmp1 - SRC0: number - SRC1: number
                            Goldilocks::op_avx(args[i_args], tmp1[args[i_args + 1]], numbers_[j][args[i_args + 2]], numbers_[j][args[i_args + 3]]);
                            i_args += 4;
                            break;
                        }
                        case 14: {
                        ...
                        case 17: {
                            ...
                            Goldilocks3::op_31_avx(args[i_args], tmp3[args[i_args + 1]], (Goldilocks3::Element_avx &)bufferT_[nColsStagesAcc[args[i_args + 2]] + args[i_args + 3]], numbers_[j][args[i_args + 4]]);
                            i_args += 5;
                            break;
                        }
                        case 18: {
                        ...
                        case 21: {
                            ...
                            Goldilocks3::op_31_avx(args[i_args], tmp3[args[i_args + 1]], tmp3[args[i_args + 2]], numbers_[j][args[i_args + 3]]);
                            i_args += 4;
                            break;
                        }
                        case 22: {
                        ...
                        case 25: {
                            ...
                            Goldilocks3::op_31_avx(args[i_args], tmp3[args[i_args + 1]], challenges[args[i_args + 2]], numbers_[j][args[i_args + 3]]);
                            i_args += 4;
                            break;
                        }
                        case 26: {
                        ...
                        case 29: {
                            ...
                            Goldilocks3::op_31_avx(args[i_args], tmp3[args[i_args + 1]], subproofValues[args[i_args + 2]], numbers_[j][args[i_args + 3]]);
                            i_args += 4;
                            break;
                        }
                        case 30: {
